...
const VoiceoverChart = ({ chartData, loading }) => {
  const classes = ChartStyles();
  const dispatch = useDispatch();

  const supportedLanguages = useSelector(
    (state) => state.getSupportedLanguages.voiceoverLanguages
  );
    ...
    if (chartData) {
      chartData.forEach((item) => {
        ...
        item.data.forEach((value) => {
          taskCount = taskCount + value.voiceovers_completed.value;
          taskDuration = taskDuration + value.voiceover_duration.value;
        });
  ...
  useEffect(() => {
    ...
    if (asPer === "Count") {
      res?.sort(
        (a, b) => b.voiceovers_completed.value - a.voiceovers_completed.value
      );
    } else {
      res?.sort(
        (a, b) => b.voiceover_duration.value - a.voiceover_duration.value
      );
    }
    ...
    res?.forEach((item) => {
      const { tgt_language, voiceovers_completed, voiceover_duration } = item;

      totalCountByOrgLang += voiceovers_completed.value;
      totalDurationByOrgLang += voiceover_duration.value;

      if (asPer === "Count") {
        arr.push({
          [tgt_language.label]: tgt_language.value,
          [voiceovers_completed.label]: voiceovers_completed.value,
        });
      } else {
        arr.push({
          [tgt_language.label]: tgt_language.value,
          [voiceover_duration.label]: voiceover_duration.value,
        });
      }
    });
  ...
  useEffect(() => {
    const apiObj = new FetchSupportedLanguagesAPI("VOICEOVER");
      ...
      <Box>
        <Typography variant="h4" className={classes.heading}>
          ...
          <Typography variant="body1">
            Total count of completed Subtitle Voiceover Tasks for a given
            language
          </Typography>
        </Typography>

        <Typography variant="body2">
          Note : The voiceover is done on top of the translation
        </Typography>
      </Box>
      ...
      <Paper>
          ...
          {asPer === "Count" ? (
            <Box className={classes.topBarInnerBox}>
              <Typography style={{ fontSize: "0.875rem", fontWeight: "400" }}>
                Total count of completed Voiceover Tasks
              </Typography>
            ...
            <Box className={classes.topBarInnerBox}>
              <Typography style={{ fontSize: "0.875rem", fontWeight: "400" }}>
                Total duration of completed Voiceover Tasks
              </Typography>
              <Typography style={{ fontSize: "1.125rem", fontWeight: "400" }}>
                {sourceData
        ...
        >
          <Typography variant="h6">
            Number of Voiceovers per language in
          </Typography>
            ...
            <ResponsiveContainer width={"100%"} height={600}>
              ...
              >
                ...
                <Bar
                  ...
                  dataKey={
                    asPer === "Count"
                      ? "VoiceOver Tasks Count"
                      : "VoiceOver Duration (Hours)"
                  }
                  cursor="pointer"
                  radius={[8, 8, 0, 0]}
                  maxBarSize={65}
